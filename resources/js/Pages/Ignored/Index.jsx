...
export default function IgnoredIndex({ entries, filters, meta, reasonOptions }) {
    ...
    const unignore = (row) => {
        if (row.prospect_id) {
            router.delete(`/prospects/${row.prospect_id}/ignore`, { preserveScroll: true });
            return;
        }
            ...
            <main className="page page-wide">
                ...
                <FilterBar onSubmit={submitFilters}>
                    ...
                    <div className="filter-action">
                        ...
                        <Link href="/ignored" className="micro">Reset</Link>
                    </div>
                </FilterBar>
                        ...
                        <tbody>
                            {entries.map((row) => (
                                <tr
                                    ...
                                    onClick={() => row.prospect_id && router.visit(`/prospects/${row.prospect_id}`)}
                                >
                                    <td className="biz">
                                        {row.business_name ?? (
                                            <span className="micro" title={row.place_id}>Unknown business</span>
                                        )}
                                        {row.niche && row.city && (
                                            <div className="micro">{row.niche} · {row.city}</div>
                                        )}
                                    </td>
                                    <td>
